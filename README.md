...
> 仓库地址：<https://github.com/bingguo-suibingbing/cpp-notes>
...
| 章节 | 示例数 | 代码行数 | NOTES.md 行数 |
|---|---|---|---|
| `01-basics` | 9 | 2529 | 653 |
| `02-oop` | 9 | 3746 | 856 |
| `03-modern` | 9 | 3523 | 394 |
| `04-templates` | 8 | 1354 | 903 |
| `05-stl` | 10 | 6903 | 940 |
| `06-algorithms` | 10 | 16077 | 601 |
| `07-engineering` | 6 | 2876 | 743 |
| **合计** | **61** | **37,008** | **5,090** |

验证状态（本机实测）：

- `build.ps1 -Clean -WX`：**61 / 61 编译通过，零警告**（`/W4 /WX /permissive- /utf-8 /std:c++20`）。
- 全部 61 个示例实际运行：**退出码全部为 0，无崩溃**。
- `07-engineering` 独立 CMake 工程：配置 + 构建成功，`ctest` **2 / 2 测试通过**。
- `08-boost`：用 Boost 1.92 编译并运行成功。

> 注：示例里的性能数字都是在 **Debug** 配置下现场测出来的。Debug 的数字本身不重要（没有优化、容器边界检查全开），重要的是**趋势和数量级**；想认真比较性能请用 `build.ps1 -Config Release` 或 CMake 的 Release 配置重跑。
...
- **修正了原笔记中的事实性错误**。每章 `NOTES.md` 里都有一份「原笔记纠错清单」表格，逐条记录「原来怎么写 → 问题是什么 → 正确写法 → 为什么」。`01-basics` 一章就整理了 41 条。
- **统一了构建方式、代码风格与注释规范**：一个 `.cpp` = 一个独立可执行文件，全仓库统一编译选项，统一用 `std::` 限定、统一中文标点。
- **补齐了原来缺失的内容**：面向对象、现代 C++、数据结构与算法、工程实践这四块在原素材里基本是空白，属于新写。
- **把「结论」写进 `NOTES.md`，把「证据」留在代码里**：代码负责跑出结果和实测数字，文档负责给工程建议和决策表。

原始素材保留在工作区（`模块/`、`模板/`、`boost/`、`math.h/`），未删除，作为对照。
...
- 全部验证都在 **Windows + MSVC 14.51 + C++20** 上做的。代码本身尽量写成标准 C++，但 GCC/Clang 路径**没有实测过**（CMake 里已经写好对应的警告选项）。
- 下面这些工具本机**没装**，文档里提到时都标了「需要自行安装」：`clang-format`、`clang-tidy`、`ninja`。
- `07-engineering` 的 `CMakePresets.json` 里基于 Ninja 的预设路径没有实测（本机无 Ninja）；实测通过的是 Visual Studio 生成器那条路径。
- 部分示例里的性能对比在 Debug 下会被优化级别和调试检查干扰，**绝对值不可当真**。
